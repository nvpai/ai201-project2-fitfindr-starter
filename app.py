...
def handle_query(user_query: str, wardrobe_choice: str) -> tuple[str, str, str]:
    ...
    # Stretch: append the price-fairness check.
    price_check = session.get("price_check")
    if price_check:
        listing_text += f"\n\n💰 Price check: {price_check['message']}"

    # Stretch: append trending styles for this size.
    trends = session.get("trends")
    if trends:
        trend_str = ", ".join(f"{t['style']} ({t['count']})" for t in trends)
        listing_text += f"\n\n🔥 Trending in your size: {trend_str}"
